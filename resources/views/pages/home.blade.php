...
@section('content')
<div class="px-4 md:px-16 grid grid-cols-1 items-start mt-10 space-y-4">
    <div class="w-full overflow-x-scroll flex flex-row gap-2 pb-2">
        @if(count($data['data']) > 0)
            ...
            @foreach($uniqueGames as $game)
                <a href="#" class="text-white game-link whitespace-nowrap px-4 py-2 bg-gray-700 hover:bg-gray-800 rounded-md" data-game="{{ $game['game'] }}">{{ $game['game'] }}</a>
            @endforeach
        @else
            <p>No data available</p>
        @endif
    </div>
    <form id="pay-form" action="{{ route('place-order') }}" method="POST" class="space-y-4">
        @csrf
            ...
            @if(count($data['data']) > 0)
                @foreach($data['data'] as $feature)
                    @if($feature['status'] === "available")
                        <div class="p-2 w-full bg-gray-50 dark:bg-gray-900 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 game-container" data-game="{{ $feature['game'] }}">
                            <div class="flex flex-col">
                                ...
                                <label class="w-full text-lg font-medium text-gray-900 dark:text-gray-300">Rp {{ number_format($feature['price']['basic'], 0, ',', '.') }}</label>
